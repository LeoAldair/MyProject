...
class ProfileFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        myRef.child("Users").get().addOnSuccessListener { response ->
            ...
            resmap.forEach { user ->
                if(!user.isNullOrEmpty() && user["name"] == usuario){
                    binding.tvUserFirstNameData.setText(user["name"].toString())
                    binding.tvUserLastNameData.setText(user["last"].toString())
                    binding.tvUserEmailData.setText(user["email"].toString())
                    binding.tvUserQuantityData.setText(user["wishlist"].toString())
                    val level = user["level"].toString().toInt()
                    if(level<100) {
        ...
        binding.btnUpdateUser.setOnClickListener {
            //val database = Firebase.database
            //val myRef = database.reference
           // myRef.child("Users").child("1").setValue(User("Leonardo Aldair","Resendiz Pantoja","leo@gmail.com",0,80 ))
           // myRef.child("Users").child("2").setValue(User("Paulina","Ruiz Reyes","paupau@gmail.com",0,150 ))
           // myRef.child("Users").child("3").setValue(User("Mauricio","Nieto","mauN@gmail.com",0,220 ))
